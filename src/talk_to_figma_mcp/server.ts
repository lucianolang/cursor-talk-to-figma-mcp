...
const hasScheme = /^wss?:\/\//.test(serverUrl);
const isLocalServer = /^(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/i.test(serverUrl);
const WS_URL = hasScheme
  ? serverUrl
  : isLocalServer
    ? `ws://${serverUrl}`
    : `wss://${serverUrl}`;
...
server.registerTool(
  ...
  async ({ collectionId }: { collectionId: string }): Promise<any> => {
    try {
      const result = await sendCommandToFigma("find_broken_variable_aliases", { collectionId }) as {
        broken?: unknown[];
        brokenCount?: number;
      };
...
type CommandParams = {
  ...
  find_broken_variable_aliases: { collectionId: string };
